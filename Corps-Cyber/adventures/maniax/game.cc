...
Game::Game() throw() :
  ...
  client_list()
{
  _name = gettext("The Maniax");
  _description = gettext("A simple introduction adventure for the system \"Cyber\".");
  _system = new System();
  game_type = GTMultiPlayer;

  pthread_mutex_init(&client_list_lock, 0);

  _system->_obj_is_ready(orb->getBOA());
}
...
string Game::_join(Client_Wrapper client) throw(CorbaException)
{
  try
	...
	CH_History_Wrapper(client.get_handler("HISTORY", 0));
      ...
      entry.history.append_text("Welcome to \"The Maniax\" - a CORPS adventure (using system Cyber)"); //FIXME: language
